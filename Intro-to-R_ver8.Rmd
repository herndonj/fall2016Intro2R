...
* load a file [via http](#http-load) -- which you will do later in this workshop.  
...
In this section, practice working with your filesystem settings and assigning those values to object names.
```{r}
DIR <- getwd()
...
If you upload data using any of the following methods in RStudio you can ignore the preceding [parts about manipulating the files system](#getwd) and directory structure of your computing environment.
...
## View() 
Displays your data in a grid style within the Source Editor pane ([screenshot](http://paulyeo21.github.io/cell_bio_gen_lab/images/331f4ea3.rstudio_pane.png).) This is not a reproducible method but many people like to see their data, or at least a grid-like representation of a piece of their data.  For that use the *View()* function; that's View with a *captical V*.
```{r}
View(unempdata) 
```
...
This is contextually related to tables.  Let's display the unique values for the two variables in the data frame. {#http-load}
```{r}
