...
The follow is draft document presenting an introductory workshop in based on a [thought document](https://docs.google.com/document/d/1FUdzDSje_b-0n0fxNrrXiJ11kUC5sNzhYdYmyXu_GFU/edit).  The intended audience are students who signed up for the [Data & Visualization Department](http://library.duke.edu/data/) [Workshop Series](http://library.duke.edu/data/news): _Introduction to R_.  The first section of this document takes care of loading preliminary libraries.  This preliminary step is documented for those learning R. **Skip ahead**  to [Begin - Intro to R](#begin).

## Library Packages
Load all the packages for this tutorial. Strictly speaking this is a two step process:  (1) install the package; (2) load the package.  I'll include the install commands for [reproducibility](https://en.wikipedia.org/wiki/Reproducibility), but you can probably skip to simply loading the packages.  If any library  doesn't load, you can uncomment the "install.packages" command for that package (remove the # preceding the relevant "install.packages" command.)  Then, rerun the relevant code-chucks.  

A **note** about the number of packages.  The necessary packages that make up the bulk of concepts taught in this workshop are: **readr**,  **dplyr**, and **ggplot2**.  stringr, and stringi are for string manipulation but we won't go into great depth as to their use.  The rmarkdown package is a utility that allows me to compose this workshop and create code-chunks.  All of the code-chunks can be run just as well in a raw 'r' script -- we'll talk about that in the workshop.
...
library(stringi)    # working with strings:  regex & capture groups
...
R is a popular programming environment with embedded statistical capabilities.  The R language is also popular as a data science toolset.  If you want to read more about the origins and definitions of **R**, try starting at the [Wikipedia entry](https://en.wikipedia.org/wiki/R_(programming_language)).  Our focus in this introduction will develop your skills working in the [RStudio](https://www.rstudio.com/) IDE (development environment.)  

## Sample Data
See [Download Sample Data](#sample) in Section 4.1
...
As you can see the object orientation of R allows you to be specific about object characteristics.  These characteristics are called _Data Types_ or _Data Structures_. This level of specificity can yield sophisticated outcome but may also lead to confusion, especially with those learning R.  For this workshop we'll stick with two very common data types.

## Vectors and Data Frames
 
* **Vectors** are one dimensional arrays which must be all of the same mode:  _Numeric_, _Complex_, _Logical_, or  _Character_
...
For now, remember you can only store one data type in a Vector *but* you can store multiple vector types in a data frame.  I encourage you to read more about Data Types, later.
...
## Download sample data {#sample}
### Steps
1. Download the sample [UnempData.xlsx](http://foo.com) **jrljrl*** file to your download directory.
2. Move or [upload](#upload) the file to your working directory as identified by the command [getwd()](#getwd)
3. Load that data using assignments and the `read_csv` function


## R and your file system
### getwd()
*First*, identify the R's default data directory.
```{r}
getwd()
```
...
e.g. to read data into a data frame
```{r}
DIR <- getwd()
filename <- "UnempData.csv"
fullpath <- paste(DIR, "/", filename, sep = '')        # paste() to concatenate vectors

fullpath                          #display the value of fullpath in the console
```

## Where is my data? {#upload}
If you upload data using any of the following methods in RStudio then you can ignore the preceding [parts about manipulating the files system](#getwd) and directory structure of your computing environment.
...
In this example we use the read_csv() function, part of the [readr](https://github.com/hadley/readr#readr) library.  We recommend readr because it automatically imports character data as characters.  This is an improvement over the default "base R"" approach which automatically converts characters to factors whenever R thinks that's a good idea.  This happens too often in our opinion, hence we recommend using readr.
...
There are many other types of data you may want to load.  Or you may want to load a data set from a URL.  Many loading packages are available to support many methods of loading data.  There is also a download package that will download data into your R environment.  Some of the more common data loading methods are supported by the following packages:
...
## View() 
Displays your data in a grid style within the "Source"" pane.  This is not a reproducible method but many people like to see their data, or at least a grid-like representation of a piece of their data.  For that use the *View()* function; that's View with a *captical V*.
```{r}
View(unempdata) 
```
...
[dplyr](https://github.com/hadley/dplyr#dplyr) is a broadly adopted library package to support data manipulation (aka data cleaning, data wrangling, data janitor, data engineering, etc.)

## tbl_df
A convenient summary of the characteristics of a data frame object
```{r}
tbl_df(unempdata)
```
...
* filter out the 8 cylinder vehicles with select
* select only a few specific columns for my data frame
* add a column by calculating new custom values for a new variable
```{r}
...
Put the data in a custom order in a data frame display
```{r}
arrange(four.six, cyl, desc(mpg), desc(hp), wt, wtdisp)
```

## Regex & Capture Groups
String manipulation.  Let's use some advanced regex to set the data up for the [Visualization](#viz) section.
...
This is contextually related to tables.  Let's display the unique values for the two variables in the data frame.
```{r}
...
* [RStudio in the Duke Cloud](https://vm-manage.oit.duke.edu/containers/rstudio)(docker.)  Duke OIT manages a cloud version of RStudio for the convenience Duke University Community.  The advantage is that you don't have to install anything.  RAM is limited to 2GB.  If you are exceeding that limit,  the tools we're using here are easily installed into your own computing evironment.  R and [RStuido](https://www.rstudio.com/) are free and/or open source.

## Exercises

Reinforce your learning with these exercises and two-minute video tutorials
...
1. [Data Carpentry](http://www.datacarpentry.org/R-ecology-lesson/) - Text-based lessons aimed at the research environment for people with no programming experience
2. [DataCamp.com](https://www.datacamp.com/courses) - This free, interactive, online workshop tool helps you learn master the basics
3. [Lynda Online Training](https://www.lynda.com/R-training-tutorials/1570-0.html) - [Duke login](http://oit.duke.edu/comp-print/training/online/lynda.php)
...
[^1]: [First Steps in R](http://venus.ifca.unican.es/Rintro/index.html) by Maite Ceballos (IFCA) & Nicolás Cardiel (UCM)
