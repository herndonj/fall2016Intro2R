...
## Hadleyverse
These days, much of what we try to accomplish within the data universe requires reshaping the data.  The various reshaping activies occur with almost astounding frequency among datasmith.  Much of this activity have been identified as the _Split-Apply-Combine Strategy_[^7] by [Hadley Wickam](http://hadley.nz/).  To do that end, Hadley Wickham has authored or coauthored many packages to augment the base-R functionality in support of this data reshaping.  Hadley's efforts have been so broadly adopted that many people call this suite of packages "Hadleyverse".  We will use many of the Hadleyverse packages in this workshop.  A selection of Hadleyverse packages have been selected and applied in the code-chunk above.  (***jrljrl*** mcgrider)
...
In the example below, I call the `colnames()` function before and after the `rename()` function so you can see the effect.
```{r}
...
`unique()` is contextually related to `table()` in that it reveals the unique values within a variable.  To  demonstrate the `table()` function, let's use an example and example data made available from Kelly Black's R Tutorial[^9]
```{r}
...
tbl_df(smokerdata)            # Note the structe of the data
...
## plot() - Scatter Plot
`plot()` will generate a scatter plot.  To demonstrate this we'll introduce a new dataset, Egar Anderson's Iris Data.  The data contains the measurements in centimeters of various flowers: variable sepal and petal measurements in length and width for 50 flowers from each of 3 species of iris. You can find more information about this dataset by typing `?iris` in the console pane.  The scatter plot example comes from Peter Cock's guide to plotting in [R][^11]. 
```{r}
...
[^9]: [R Tutorial](http://www.cyclismo.org/tutorial/R/tables.html#graphical-views-of-tables) by Kelly Black
