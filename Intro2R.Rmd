...
### Load Data
```{r get data}
# mtcars <- read_csv("https://ndownloader.figshare.com/files/5960016?private_link=408e6198afbfd3923e71")
...
* To get help, use precede command with '?' 
    * `?cars`
...
1. load dataset  `WestUnionData <- read_csv("http://figshare.org/joel/foo.csv")`
2. what data type is WestUnionData?
...
```{r missingData}
nadata <- c("cat", "dog", NA_character_, "fish", "snake")
anyNA(nadata)
```
